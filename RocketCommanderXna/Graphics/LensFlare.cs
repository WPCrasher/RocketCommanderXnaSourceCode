// Project: RocketCommanderXna, File: LensFlare.cs
// Namespace: RocketCommanderXna.Graphics, Class: LensFlare
// Path: C:\code\RocketCommanderXna\Graphics, Author: Abi
// Code lines: 377, Size of file: 10,63 KB
// Creation date: 07.09.2006 05:56
// Last modified: 07.10.2006 14:00
...
using Material = RocketCommanderXna.Graphics.Material;
...
namespace RocketCommanderXna.Graphics
{
	...
	public class LensFlare : IDisposable
	{
		...
		/// </summary>
		private static int ScreenFlareSize = 225;//250;
		...
		protected struct FlareData
		{
				...
				float setScale, Color setColor)
			{
				...
				scale = setScale;
				color = setColor;
			} // FlareData(setType, setPosition, setScale)
		} // struct FlareData
		...
		protected FlareData[] flareTypes = new FlareData[]
		{
			...
			new FlareData(
			GlowFlareType, 1.0f, 2.6f, new Color(255, 255, 200, 150)),
			//new FlareData(
		...
		{
			ScreenFlareSize = 225/*250*/ * BaseGame.Width / 1024;
			...
			// Mix in a little white into the sun color, will appear to weak else
			sunColor = ColorHelper.InterpolateColor(
				Color.White, sunColor, 0.5f);

			foreach (FlareData data in flareTypes)
			{
				int size = (int)(ScreenFlareSize * data.scale);
				flareTextures[data.type].RenderOnScreen(
					//tst: BaseGame.UI.Logos.RenderOnScreen(
					new Rectangle(
